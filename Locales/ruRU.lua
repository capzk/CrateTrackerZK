...
localeData["TrajectoryPredictionMatchedUnified"] = "[%s] Точка сброса определена: %s";
localeData["TrajectoryPredictionCandidatesAmbiguous"] = "[%s] Есть несколько возможных точек падения:";
...
localeData["SettingsMapSelection"] = "Выбор карт";
...
localeData["SettingsTrajectoryPredictionTestTooltip"] = "При включении будут показываться локальные подсказки прогноза траектории и метка точки падения на карте.";
localeData["SettingsTrajectoryTeamAlert"] = "Отправлять прогноз в группу";
localeData["SettingsTrajectoryTeamAlertTooltip"] = "При включении успешный прогноз будет отправляться в чат группы или рейда.";
...
2. Параметры страницы настроек сообщений:
   ...
   Режим лидера: при включении видимые уведомления о сбросе и автооповещения в рейде по возможности идут в предупреждение рейда; без прав они возвращаются в обычный рейдовый чат
   Уведомления группы: при включении сообщение отправляется в группу или рейд
